...
var GetAddrInfo = function GetAddrInfo() {
  return function doGetAddrInfo(info, next) {
    ...
    var addresses = [];
    dns_sd.dnsServiceGetAddrInfo(addressInfoGetter.serviceRef, 0, 0, 0, info.host,
        function(sdRef, adrFlags, adrIface, errorCode, hostname, address, context) {
          ...
          if (error) {
            ...
            if ( ! (dns_sd.kDNSServiceFlagsMoreComing & adrFlags)) {
...
Browser.defaultResolverStack = [
  ServiceResolve()
, 'dnsServiceGetAddrInfo' in dns_sd ? GetAddrInfo() : OldschoolLookup()
]
